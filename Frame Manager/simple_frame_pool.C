...
SimpleFramePool::FrameState SimpleFramePool::get_state(unsigned long _frame_no) {
    unsigned int bitmap_index = _frame_no / 8; // this returns entire byte of 8 frame statuses
    unsigned char mask = 0x1 << (_frame_no % 8); // this will shift a mask till desired frame bit in that byte
